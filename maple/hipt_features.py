...
from . import hipt_4k
from .hipt_utils import eval_transforms
...
def compute_tissue_mask(
    ...
    for i in range(max_iter):
        old_mask = mask.copy()
        try:
            cv.grabCut(image, mask, None, bgd_model, fgd_model, 1, cv.GC_INIT_WITH_MASK)
...
def get_embeddings(img, pretrained=True, device='cuda'):
    ...
    if pretrained:
        model256_path = str(resources.files(__name__).joinpath('../data/hipt_model/vit256_small_dino.pth'))
        model4k_path = str(resources.files(__name__).joinpath('../data/hipt_model/vit4k_xs_dino.pth'))
    model = hipt_4k.HIPT_4K(
...
def smoothen(x, size, kernel='gaussian', backend='cv', mode='mean',
...
def get_hipt_features(img, locs, rad, pixel_size_raw, pixel_size=0.5, pretrained=True, device='cuda'):
    ...
    if rad > 16:
        cls_sub2 = np.stack([rearrange(cls_sub1[(int(np.ceil((locs1['4'].iloc[i] - rad) / 16))): \
            (int(np.floor((locs1['4'].iloc[i] + rad) / 16))), (int(np.ceil((locs1['5'].iloc[i] - rad) / 16))): \
            (int(np.floor((locs1['5'].iloc[i] + rad) / 16)))], 'h w c -> (h w) c').mean(0) for i in range(locs1.shape[0])])
    else:
        cls_sub2 = cls_sub1[(locs1['4'].iloc[:] / 16).round().astype('int'), (locs1['5'].iloc[:] / 16).round().astype('int'), :]
